...
export function createRitualTapestryV3_GLSLOnlyPreset(
  scene: unknown,
...
void main(){
  float t=u.time;
  float env=sat(u.rms*0.60 + u.bass*0.55 + u.beat*0.55);
  float hi=sat(u.high*0.95 + u.mid*0.35);
  ...
  float swirl = 0.10*sin(t*0.25)*(0.35+0.65*sat(u.mid));
  p=rot(p, swirl + 0.05*v);

  float inkv=sat(u.ink);
  ...
  float mand=mandala(p*1.05, t*(0.95+0.6*sat(u.mid)), sc, rc);
  ...
  float eyePulse=(0.45 + 1.05*u.shock) * (0.35 + 0.85*hi);
...
`;
  ...
  const SHADER_NAME = "ritualV3GLSL";
  (BABYLON.Effect as any).ShadersStore[SHADER_NAME + "VertexShader"] = vertGLSL;
  (BABYLON.Effect as any).ShadersStore[SHADER_NAME + "FragmentShader"] = fragGLSL;
  ...
  function makeMat(name: string) {
    const mat = new BABYLON.ShaderMaterial(
      ...
      SHADER_NAME,
      {
        ...
        uniformBuffers: ["scene", "u"],
        shaderLanguage: BABYLON.ShaderLanguage.GLSL,
      } as any,
    );
    const sceneUBO = bjsScene.getSceneUniformBuffer?.();
    if (sceneUBO) {
      mat.setUniformBuffer("scene", sceneUBO);
    }
    return mat;
  }

  function createRitualUBO(engine: Engine, variant: number) {
    const ubo = new BABYLON.UniformBuffer(engine);
    ubo.addUniform("time", 1);
    ubo.addUniform("bass", 1);
    ubo.addUniform("mid", 1);
    ubo.addUniform("high", 1);
    ubo.addUniform("rms", 1);
    ubo.addUniform("beat", 1);
    ubo.addUniform("intensity", 1);
    ubo.addUniform("ink", 1);
    ubo.addUniform("shock", 1);
    ubo.addUniform("variant", 1);
    ubo.create();

    // initialize once (prevents bindGroup undefined)
    ubo.updateFloat("time", 0);
    ubo.updateFloat("bass", 0);
    ubo.updateFloat("mid", 0);
    ubo.updateFloat("high", 0);
    ubo.updateFloat("rms", 0);
    ubo.updateFloat("beat", 0);
    ubo.updateFloat("intensity", 1);
    ubo.updateFloat("ink", 0);
    ubo.updateFloat("shock", 0);
    ubo.updateFloat("variant", variant);
    ubo.update();
  ...
  const uboCenter = createRitualUBO(engine, 0);
  const uboLeft = createRitualUBO(engine, 1);
  const uboRight = createRitualUBO(engine, 2);

  // bind UBOs to each material
  matCenter.setUniformBuffer("u", uboCenter);
  matLeft.setUniformBuffer("u", uboLeft);
  matRight.setUniformBuffer("u", uboRight);
  console.log("UBO bound?", !!(matCenter as any)._uniformBuffers?.u);
  ...
  return {
    update(audio: AudioLike, dt: number, blend: Blend) {
      ...
      function push(ubo: UniformBuffer, variant: number) {
        ubo.updateFloat("time", time);
        ubo.updateFloat("bass", bass);
        ubo.updateFloat("mid", mid);
        ubo.updateFloat("high", high);
        ubo.updateFloat("rms", rms);
        ubo.updateFloat("beat", beat);
        ubo.updateFloat("intensity", intensity);
        ubo.updateFloat("ink", ink);
        ubo.updateFloat("shock", shock);
        ubo.updateFloat("variant", variant);
        ubo.update();
      }
