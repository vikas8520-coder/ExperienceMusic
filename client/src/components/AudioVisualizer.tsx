...
const EarthShader = {
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      float continent = fbm(pos * 2.5 + vec3(0.0, 0.0, 1.0));
      float detail = snoise(pos * 8.0 + vec3(3.0, 1.0, 2.0)) * 0.15;
      float landMask = smoothstep(-0.05, 0.15, continent + detail);

      float iceCap = smoothstep(0.75, 0.95, abs(pos.y));
      ...
      vec3 lowland = vec3(0.08, 0.28, 0.05);
      vec3 highland = vec3(0.22, 0.18, 0.08);
      vec3 mountain = vec3(0.35, 0.30, 0.22);
      float elevation = smoothstep(0.0, 0.4, continent + detail);
      vec3 land = mix(lowland, highland, elevation);
      land = mix(land, mountain, smoothstep(0.6, 0.9, elevation));

      vec3 surface = mix(ocean, land, landMask);
      surface = mix(surface, vec3(0.9, 0.93, 0.97), iceCap);

      float cloudNoise = fbm(pos * 3.0 + vec3(uTime * 0.02, uTime * 0.01, 0.0));
      float clouds = smoothstep(0.05, 0.35, cloudNoise);
      clouds *= (1.0 - iceCap * 0.5);
      surface = mix(surface, vec3(0.95, 0.97, 1.0), clouds * 0.7);

      vec3 sunDir = normalize(vec3(0.8, 0.4, 0.6));
      float diff = clamp(dot(vWorldNormal, sunDir), 0.0, 1.0);
      float ambient = 0.12;
      float wrap = clamp(dot(vWorldNormal, sunDir) * 0.5 + 0.5, 0.0, 1.0);

      vec3 lit = surface * (ambient + diff * 0.75 + wrap * 0.15);

      float spec = pow(clamp(dot(reflect(-sunDir, vWorldNormal), viewDir), 0.0, 1.0), 48.0);
      lit += vec3(0.6, 0.7, 1.0) * spec * (1.0 - landMask) * 0.4;

      vec3 atmosColor = vec3(0.3, 0.55, 1.0);
      lit += atmosColor * fresnel * 0.5;

      float kick = uKick;
      lit += vec3(0.8, 0.9, 1.0) * kick * 0.3 * fresnel;

      float bassPulse = uBass * 0.15;
      lit *= 1.0 + bassPulse;

      float nightGlow = smoothstep(0.1, -0.2, diff);
      float cityNoise = smoothstep(0.3, 0.7, snoise(pos * 20.0)) * landMask;
      vec3 cityLights = vec3(1.0, 0.85, 0.5) * cityNoise * nightGlow * 0.3;
      cityLights *= (1.0 + uEnergy * 0.8);
      lit += cityLights;
...
const AtmosphereShader = {
  ...
  fragmentShader: `
    ...
    void main() {
      vec3 viewDir = normalize(cameraPosition - vWorldPosition);
      float rim = 1.0 - clamp(dot(viewDir, vWorldNormal), 0.0, 1.0);
      float glow = pow(rim, 3.0);
      float outerFade = pow(rim, 1.5);

      vec3 atmosColor = mix(uColor1, vec3(0.3, 0.6, 1.0), 0.5);
      vec3 col = atmosColor * glow * (1.2 + uBass * 0.4);

      float alpha = outerFade * (0.35 + uEnergy * 0.15);
      alpha *= smoothstep(0.0, 0.3, rim);
